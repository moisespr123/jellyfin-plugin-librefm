namespace Jellyfin.Plugin.Librefm
{
    ...
    public class ServerEntryPoint : IHostedService, IDisposable
    {
        ...
        private LibrefmApiClient _apiClient;
        ...
        async void UserDataSaved(object sender, UserDataSaveEventArgs e)
        {
            ...
            var librefmUser = Utils.UserHelpers.GetUser(e.UserId);
            if (librefmUser == null)
            {
            ...
            if (string.IsNullOrWhiteSpace(librefmUser.SessionKey))
            {
            ...
            {
                if (!librefmUser.Options.Scrobble)
                {
                    _logger.LogDebug("{0} does not want to scrobble", librefmUser.Username);
                    return;
                }
                if (!librefmUser.Options.AlternativeMode)
                {
                    _logger.LogDebug("{0} does not use AlternativeMode", librefmUser.Username);
                    return;
                }
                ...
                await _apiClient.Scrobble(item, librefmUser).ConfigureAwait(false);
            }
        }


        /// <summary>
        /// Let libre.fm know when a track has finished.
        ...
        private async void PlaybackStopped(object sender, PlaybackStopEventArgs e)
        {
            ...
            // Required checkpoints before scrobbling noted at https://www.libre.fm/api
            ...
            {
                _logger.LogDebug("{0} - played {1}%, Libre.Fm requires minplayed={2}% . played {3} ticks of minimumPlayTimeToScrobbleInTicks ({4}), won't scrobble", item.Name, playPercent, minimumPlayPercentage, e.PlaybackPositionTicks, minimumPlayTimeToScrobbleInTicks);
                return;
            }
            ...
            var librefmUser = Utils.UserHelpers.GetUser(user);
            if (librefmUser == null)
            {
                _logger.LogDebug("Could not find libre.fm user");
                return;
            }
            ...
            if (!librefmUser.Options.Scrobble)
            {
                _logger.LogDebug("{0} ({1}) does not want to scrobble", user.Username, librefmUser.Username);
                return;
            }
            if (librefmUser.Options.AlternativeMode)
            {
                _logger.LogDebug("{0} uses AlternativeMode", librefmUser.Username);
                return;
            }
        ...
        /// <summary>
        /// Let Libre.fm know when a user has started listening to a track
        /// </summary>
        private async void PlaybackStart(object sender, PlaybackProgressEventArgs e)
        {
            ...
            await _apiClient.NowPlaying(item, librefmUser).ConfigureAwait(false);
        }
